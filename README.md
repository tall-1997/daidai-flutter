...
- 修复 Liquid Glass 主题下环境变量卡片透明时，关闭状态仍会折射显示侧滑启用、禁用和删除按钮的问题。
- 侧滑操作层仅在卡片实际向左露出的区域内绘制，保持玻璃背景效果和操作按钮视觉隔离。
- 卡片位移与操作层裁剪使用同一吸附动画，避免展开、收起和返回手势收起期间出现按钮闪现。
- Android 继续沿用 `v1.0.0` 固定 JKS 证书，支持覆盖升级；iOS 继续提供无签名 IPA。
...
- 完整保留 Multi-Cron 任务表达式、模板分组、面板时区和未知服务端枚举值。
- 引入 Session Epoch 与 Panel Scope，隔离认证、Token 刷新、REST、SSE 和跨面板页面状态。
- 增强 SSE 断线续传、事件 ID 去重、退避重连、生命周期暂停和终态处理。
- 完善任务、通知、订阅、用户、脚本、备份、Open API 等页面的错误恢复与重试。
- 增强原始日志票据续期、临时分享、文件保存和清理流程。
- 增加增强管理端点能力降级、HTTP/HTTPS 自动互试和全局任务完成通知。
- 环境变量卡支持左滑启用、禁用和删除，返回手势优先收起操作区。
- `v0.1.65` 提供临时证书 Android APK 与无签名 iOS IPA，并关闭跨签名自动更新和差分包。

### v0.1.64

- 新增按面板 URL 隔离的能力档案，通过实际端点响应识别任务视图、面板设置、版本信息和多 Python runtime 支持状态。
- 旧面板返回 404 或 405 时自动隐藏对应增强入口；超时、断网和 5xx 保留后续重试能力。
- 仪表盘优先展示系统与任务核心数据，面板标题和版本信息改为后台补充。
- 任务列表不再等待任务视图探测，依赖列表与 Python runtime 并行加载并减少重复计数请求。
- HTTP 与 SSE 认证请求使用进程内 Token 快照，减少安全存储平台通道访问。
- 并发 401 共享 Token 刷新结果，积压请求并发重放，并阻止旧面板响应清理新面板会话。
- 登录和可信会话恢复后由目标页面负责首次加载，消除重复仪表盘请求。
- 切换面板时统一失效主要页面 Provider，避免短暂显示上一面板数据。

### v0.1.63

- 修复 Pure Flat 主页面顶部状态栏区域显示黑边的问题。
- 页面纯色或背景图延伸至系统状态栏下方，业务内容继续遵守安全区。
- 状态栏保持透明，并根据浅色、深色主题自动切换系统图标亮度。
- 保持 Pure Flat 低占用路径，不增加 AppBar、半透明遮罩、模糊或玻璃渲染层。
- 增加状态栏背景层级、系统样式和零玻璃组件回归测试。

### v0.1.62

- 实时日志统一使用有界内存缓冲，同时限制行数和字符量，改善长时间运行时的内存占用。
- 修复日志自动刷新、分页和筛选请求竞争，页面进入后台时暂停周期轮询，恢复前台后按需刷新。
- Pure Flat 应用锁改用不透明遮罩，Liquid Glass 主框架减少页面级捕获层级。
- 背景图片按屏幕物理尺寸解码，并使用静态图片滤镜处理模糊，降低 GPU 与图片内存成本。
- 任务日志历史回放改用游标去重，避免大日志场景中的重复头部移动。
- Android Root 子进程移至后台执行器，并发读取输出流，增加超时和资源释放保护。
...
- 修复 Liquid Glass Slider 可空参数的编译兼容问题。

### v0.1.60

- 此标签的构建在质量门禁阶段发现编译兼容问题，正式安装包由 `v0.1.61` 提供。
...
- 移除未完成的 Android 本地面板实验功能，App 保持为远程面板客户端。
- 启动流程统一使用用户配置的面板地址。
...
- 面板地址示例：`https://panel.example.com`
- 常规接口：`/api`
- 流式接口：`/api/v1`
